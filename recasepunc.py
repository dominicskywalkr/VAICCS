...
# Modified recasepunc.py to work with VAICCS
...
class CasePuncPredictor:
    def __init__(self, checkpoint_path, lang=default_config.lang, flavor=default_config.flavor, device=default_config.device):
        # Robust checkpoint loading: newer PyTorch may default to weights_only=True
        # which prevents loading objects (like tokenizers) saved in older checkpoints.
        try:
            loaded = torch.load(checkpoint_path, map_location=device if torch.cuda.is_available() else 'cpu')
        except Exception as e:
            msg = str(e)
            if 'Weights only load failed' in msg or 'weights_only' in msg or 'Unsupported global' in msg:
                print('Warning: checkpoint requires full load; retrying with weights_only=False', file=sys.stderr)
                # Attempt to allowlist tokenizer/token classes commonly present in older checkpoints
                safe_list = []
                try:
                    import transformers.models.bert.tokenization_bert as _tb
                    if hasattr(_tb, 'BertTokenizer'):
                        safe_list.append(_tb.BertTokenizer)
                    if hasattr(_tb, 'WordpieceTokenizer'):
                        safe_list.append(_tb.WordpieceTokenizer)
                except Exception:
                    try:
                        from transformers import BertTokenizer as _bt
                        safe_list.append(_bt)
                    except Exception:
                        pass

                # Some checkpoints may have pickled classes referencing names on
                # the __main__ module (e.g., WordpieceTokenizer). To help the
                # unpickler resolve these, inject known tokenizer classes into
                # the __main__ namespace where safe to do so.
                try:
                    import sys as _sys
                    _main = _sys.modules.get('__main__')
                    if _main is not None:
                        try:
                            import transformers.models.bert.tokenization_bert as _tb2
                            for _name in ('WordpieceTokenizer', 'BertTokenizer'):
                                if hasattr(_tb2, _name) and not hasattr(_main, _name):
                                    setattr(_main, _name, getattr(_tb2, _name))
                        except Exception:
                            # best-effort; ignore if tokenizers not importable
                            pass
                except Exception:
                    pass

                try:
                    # Prefer context-manager allowlist if available (PyTorch >=2.6)
                    if hasattr(torch.serialization, 'safe_globals'):
                        try:
                            with torch.serialization.safe_globals(safe_list):
                                loaded = torch.load(checkpoint_path, map_location=device if torch.cuda.is_available() else 'cpu', weights_only=False)
                        except TypeError:
                            # Some torch versions may not accept weights_only
                            loaded = torch.load(checkpoint_path, map_location=device if torch.cuda.is_available() else 'cpu')
                    else:
                        # Fallback: try add_safe_globals (may be persistent)
                        if hasattr(torch.serialization, 'add_safe_globals') and safe_list:
                            try:
        ...
        try:
            res = self.model.load_state_dict(loaded['model_state_dict'], strict=False)
            # res may be a NamedTuple with missing_keys/unexpected_keys or a dict
            try:
                missing = getattr(res, 'missing_keys', None) or (res.get('missing_keys') if isinstance(res, dict) else None)
                unexpected = getattr(res, 'unexpected_keys', None) or (res.get('unexpected_keys') if isinstance(res, dict) else None)
            except Exception:
                missing = unexpected = None
            if unexpected:
                print(f"Warning: unexpected keys in checkpoint state_dict: {unexpected}", file=sys.stderr)
            if missing:
                print(f"Warning: missing keys in model state_dict: {missing}", file=sys.stderr)
        except Exception:
            # fallback to strict load to raise the original error
...
def generate_predictions(config, checkpoint_path):
    # Torch 2.6+ may default to weights_only=True which prevents loading
    # objects like tokenizers that were saved in older checkpoints. Try
    # loading normally, and if that fails with the "Weights only load
    # failed" error, retry with weights_only=False (trusted checkpoint).
    try:
        loaded = torch.load(checkpoint_path, map_location=config.device if torch.cuda.is_available() else 'cpu')
    except Exception as e:
        msg = str(e)
        # Heuristic: detect the specific weights-only error raised by torch
        if 'Weights only load failed' in msg or 'weights_only' in msg or 'Unsupported global' in msg:
            print('Warning: checkpoint requires full load; retrying with weights_only=False', file=sys.stderr)
            # Retry with weights_only=False to allow loading tokenizer/class objects
            try:
                loaded = torch.load(checkpoint_path, map_location=config.device if torch.cuda.is_available() else 'cpu', weights_only=False)
            except TypeError:
    ...
    model = Model(config.flavor, config.device)
    try:
        res = model.load_state_dict(loaded['model_state_dict'], strict=False)
        try:
            missing = getattr(res, 'missing_keys', None) or (res.get('missing_keys') if isinstance(res, dict) else None)
            unexpected = getattr(res, 'unexpected_keys', None) or (res.get('unexpected_keys') if isinstance(res, dict) else None)
        except Exception:
